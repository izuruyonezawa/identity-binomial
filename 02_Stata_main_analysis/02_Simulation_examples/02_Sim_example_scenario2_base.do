/* 
Presenting example simulation code to produce an estimates dataset for scenario 2 (covariate effects) 
DGM: base DGM with the most extreme parameter value (binary covariate effect of 0.08 on risk difference scale)
...
syntax [ , ///
	...
	* No treatment effect
	* Binary covariate cov has effect size of 0.08 (risk difference)
    generate byte y = rbinomial(1, 0.91 + 0.08*cov) if trt==0 
    replace y = rbinomial(1, 0.91 + 0.08*cov ) if trt==1 
    assert inlist(y,0,1) 
...
set rngstream 1
set seed 7567405
capture postclose simcheck_s2_base
capture postclose rngstates_s2_base
...
postfile simcheck_s2_base int(rep) float(trt_u se_u) int(errors N n_iter) float(trt_a se_a t0b0 t1b0 t0b1 t1b1 max trt_stan se_stan t0b0_s t1b0_s t0b1_s t1b1_s) using simcheck_s2_base_postfile, replace
postfile rngstates_s2_base int(rep) str2000(rngstate1 rngstate2 rngstate3)  ///
	using rngstates_s2_base_postfile, replace
...
forvalues i=1/`repsplus1' {
	...
	post rngstates_s2_base (`i') ("`rngstate1'") ("`rngstate2'") ("`rngstate3'")
