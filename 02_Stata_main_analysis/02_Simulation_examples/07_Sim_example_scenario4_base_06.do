...
DGM: base DGM with treatment effects of ±0.06 on risk difference scale in covariate subgroups
...
syntax [ , ///
	...
	* Treatment effects are specified for each stratum to specify interactions
    generate byte y = rbinomial(1, 0.92) if trt==1 & cov == 0
    replace y = rbinomial(1, 0.98) if trt==1 & cov == 1 
	replace y = rbinomial(1, 0.98) if trt==0 & cov == 0
	replace y = rbinomial(1, 0.92) if trt==0 & cov == 1
    assert inlist(y,0,1)
...
set rngstream 1
set seed 7567405
capture postclose simcheck_s4_base_06
capture postclose rngstates_s4_base_06
...
postfile simcheck_s4_base_06 int(rep) float(trt_u se_u) int(errors N n_iter) float(trt_a se_a t0b0 t1b0 t0b1 t1b1 max trt_stan se_stan t0b0_s t1b0_s t0b1_s t1b1_s) using simcheck_s4_base_06_postfile, replace
postfile rngstates_s4_base_06 int(rep) str2000(rngstate1 rngstate2 rngstate3)  ///
	using rngstates_s4_base_06_postfile, replace
...
forvalues i=1/`repsplus1' {
	...
	post rngstates_s4_base_06 (`i') ("`rngstate1'") ("`rngstate2'") ("`rngstate3'")
	
	if `i'>`reps' continue, break
	
	qui gendata, obs(624) rd(0.00)
	qui anadata, rep(`i') post(simcheck_s4_base_06)
}

postclose simcheck_s4_base_06
postclose rngstates_s4_base_06
