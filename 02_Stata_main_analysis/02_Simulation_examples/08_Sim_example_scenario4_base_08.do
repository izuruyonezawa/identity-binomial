...
DGM: base DGM with the most extreme parameter value (treatment effects of ±0.08 on risk difference scale in covariate subgroups)
...
set rngstream 1
set seed 7567405
capture postclose simcheck_s4_base_08
capture postclose rngstates_s4_base_08
...
postfile simcheck_s4_base_08 int(rep) float(trt_u se_u) int(errors N n_iter) float(trt_a se_a t0b0 t1b0 t0b1 t1b1 max trt_stan se_stan t0b0_s t1b0_s t0b1_s t1b1_s) using simcheck_s4_base_08_postfile, replace
postfile rngstates_s4_base_08 int(rep) str2000(rngstate1 rngstate2 rngstate3)  ///
	using rngstates_s4_base_08_postfile, replace
...
forvalues i=1/`repsplus1' {
	...
	post rngstates_s4_base_08 (`i') ("`rngstate1'") ("`rngstate2'") ("`rngstate3'")
	
	if `i'>`reps' continue, break
	
	qui gendata, obs(624) rd(0.00)
	qui anadata, rep(`i') post(simcheck_s4_base_08)
}

postclose simcheck_s4_base_08
postclose rngstates_s4_base_08
