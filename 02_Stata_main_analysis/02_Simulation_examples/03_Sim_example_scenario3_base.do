/* 
Presenting example simulation code to produce an estimates dataset in scenario 3 (treatment effects) 
DGM: base DGM with the most extreme parameter value (treatment effect of 0.08 on risk difference scale)
...
syntax [ , ///
	...
	* Treatment effect will be specified with `rd'
    generate byte y = rbinomial(1, 0.91) if trt==0 
    replace y = rbinomial(1, 0.91 + `rd') if trt==1 
    assert inlist(y,0,1)  
...
program define anadata, rclass
    ...
    * Identity-binomial model for estimating covariate effect without adjustment (used for exploratory analysis)
	capture noisily glm y cov, family(binomial) link(identity) irls asis nolog
	local cov_u = _b[cov] // covariate effect
	local se_cov_u = _se[cov] // SE for covariate effect
	...
	** Estimates
	...
	local cov_a = _b[cov] // covariate effect
	local se_cov_a = _se[cov] // SE for covariate effect
	...
	* Post everything 
	if !mi("`post'") post `post' (`rep') (`cov_u') (`se_cov_u') (`errors') (`N') (`n_iter') (`trt_a') (`se_a') (`cov_a') (`se_cov_a') (`t0b0') (`t1b0') (`t0b1') (`t1b1') (`max') (`trt_stan') (`se_stan') (`t0b0_s') (`t1b0_s') (`t0b1_s') (`t1b1_s')
end
...
set rngstream 1
set seed 7567405
capture postclose simcheck_s3_base
capture postclose rngstates_s3_base
...
postfile simcheck_s3_base int(rep) float(cov_u se_cov_u) int(errors N n_iter) float(trt_a se_a cov_a se_cov_a t0b0 t1b0 t0b1 t1b1 max trt_stan se_stan t0b0_s t1b0_s t0b1_s t1b1_s) using simcheck_s3_base_postfile, replace
postfile rngstates_s3_base int(rep) str2000(rngstate1 rngstate2 rngstate3)  ///
	using rngstates_s3_base_postfile, replace
...
forvalues i=1/`repsplus1' {
	...
	post rngstates_s3_base (`i') ("`rngstate1'") ("`rngstate2'") ("`rngstate3'")
