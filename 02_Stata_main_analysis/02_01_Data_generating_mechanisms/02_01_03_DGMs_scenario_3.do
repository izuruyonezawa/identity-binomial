/* 
Presenting programs for data-generating mechanism (DGMs) with varying parameters in scenario 3 as presented in Table 3 in the main paper
*/
...
******************************************
* DGMs in scenario 3 (treatment effects) *
******************************************
...
syntax [ , ///
	...
	* Varying treatment effects are specified with `rd'
	** Outcome probability: π - rd/2 if trt = 0, π + rd/2 if trt = 1 (risk difference scale)
	** As trt is balanced 0.5/0.5, overall outcome probability is π
    generate byte y = rbinomial(1, π - `rd'/2) if trt==0 
    replace y = rbinomial(1, π + `rd'/2) if trt==1 
    assert inlist(y,0,1) 
	// π is either 0.95 or 0.80
	// `rd' is varied from 0 to 0.08 in 0.02 increments
