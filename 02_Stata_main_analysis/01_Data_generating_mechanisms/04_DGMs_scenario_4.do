/* 
Presenting programs for data-generating mechanism (DGMs) with varying parameters  in scenario 4 as presented in Table 3 in the main paper
*/
...
*********************************************************
* DGMs in scenario 4 (treatment-covariate interactions) *
*********************************************************
...
syntax [ , ///
        obs(int 624) ///
        rd(real 0.00) ] // obs and rd are sample size and treatment effect, respectively, specified later in simulations
    clear
	...
	** Outcome probability (risk difference scale): 
	// π - r/2 if (trt==1 & cov == 0) or (trt==0 & cov == 1) 
	// π + r/2 if (trt==1 & cov == 1) or (trt==0 & cov == 0)
	** As both cov and trt are balanced 0.5/0.5, overall outcome probability is π
    generate byte y = rbinomial(1, π - r/2) if trt==1 & cov == 0
    replace y = rbinomial(1, π + r/2) if trt==1 & cov == 1 
	replace y = rbinomial(1, π + r/2) if trt==0 & cov == 0
	replace y = rbinomial(1, π - r/2) if trt==0 & cov == 1
    assert inlist(y,0,1) 
	// π is either 0.95 or 0.80
	// `r' is varied from 0 to 0.08 in 0.02 increments
